...
def handler(request):
        ...
        if not slugs['a']:
                            return Response("Missing parameter 'a'", status=400)
        ...
        if 'a' in teams:
                            ...
                            if el is not None:
                                                ...
                                                el2, vb2 = extract_tagged(t['tree'], 'secondary_silhouette', 'color_field_secondary')
            if el2 is not None:
        ...
        # Layer 3: Team C flanking pair mirrored left+right
        if 'c' in teams:
                            t = teams['c']
        ...
        # Layer 4: Team D crown top + reflected bottom
        if 'd' in teams:
                            t = teams['d']
        ...
        etree.SubElement(combined, 'rect', {
                            'x': '80', 'y': str(CY + 120), 'width': str(W - 160),
                            'height': '6', 'fill': p_b['accent'], 'rx': '3'
        })

        svg_output = etree.tostring(combined, pretty_print=True)
