        ...
        for eid in ('typography_labels', 'typography_motto'):
                    for el in tree.xpath(f".//*[@id='{eid}']"):
                        ...
                        def extract_tagged(tree, *tag_ids):
                            ...
                            def force_color(el, fill, stroke=None):
                                    for node in [el] + list(el.iter()):
                                                existing_fill = (node.get('fill') or '').lower()
                                                if existing_fill != 'none':
                                                                node.set('fill', fill)
                                                            if node.get('style'):
                                                                            node.attrib.pop('style')
                                                                        if stroke and (node.get('stroke') or '') not in ('', 'none'):
            ...
            def mirrored_pair(el, vb, el_color, cx, cy, size, gap):
                    g = etree.Element('g')
                    el_left = copy.deepcopy(el)
                    force_color(el_left, el_color)
                    x_left = cx - gap - size
                    g.append(nested_svg(el_left, vb, x_left, cy - size//2, size, size))
                    el_right = copy.deepcopy(el)
                    force_color(el_right, el_color)
                    x_right = cx + gap
                    right_svg = nested_svg(el_right, vb, x_right, cy - size//2, size, size)
                    right_g = etree.Element('g', {
                        'transform': f'scale(-1,1) translate({-(x_right*2 + size)},0)'
                    })
...
class handler(BaseHTTPRequestHandler):
        ...
        try:
            ...
            # LAYER 1: Outer frame — Team A primary_silhouette
            if 'a' in teams:
                                t = teams['a']
                            ...
                            # LAYER 2: Inner shape — Team A for depth
                            if 'a' in teams:
                                                t = teams['a']
                                                el, vb = extract_tagged(t['tree'], 'secondary_silhouette', 'color_field_secondary', 'color_field_primary')
                                                if el is not None:
                                                                        force_color(el, p_a['secondary'])
                                                                        combined.append(nested_svg(el, vb, 60, 50, W-120, H-100))

                                            # LAYER 3: Hero graphic — Team B dominant center
                                            if 'b' in teams:
                                                                t = teams['b']
                                                            ...
                                                            # LAYER 4: Flanking pair — Team C mirrored left+right
                                                            if 'c' in teams:
                                                                                t = teams['c']
                                                                                el, vb = extract_tagged(t['tree'], 'sub_graphic_a', 'star_cluster', 'badge_element', 'hero_graphic')
                                                                                if el is not None:
                                                                                                        pair = mirrored_pair(el, vb, p_c['primary'], CX, CY + 20, 130, 20)
                                                                                                        combined.append(pair)

                                                                            # LAYER 5: Top crown — Team D top and reflected bottom
                                                                            if 'd' in teams:
                                                                                                t = teams['d']
                                                                                                ...
                                                                                                if el is not None:
                                                                                                                        ...
                                                                                                                        bot_g = etree.SubElement(combined, 'g', {
                                                                                                                            'transform': f'scale(1,-1) translate(0,{-H})'
                                                                                                                            })
            ...
            # Corner accent dots
            for cx_dot, cy_dot in [(140, 160), (W-140, 160), (140, H-180), (W-140, H-180)]:
                                etree.SubElement(combined, 'circle', {
                                                        'cx': str(cx_dot), 'cy': str(cy_dot), 'r': '12',
                                                        'fill': p_c['accent']
                                })
