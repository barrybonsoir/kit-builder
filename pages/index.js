...
export default function Home() {
  ...
  const drawVariableShard = (ctx, img, x, y, size, rotation, shapeType, wandSeed, cropScale) => {
    ...
    // Scale 1.0 = Full identity visible / Scale 4.0 = Aggressive color texture
  ...
  const generateMark = async () => {
    ...
    for (let l = 0; l < 12; l++) {
      ...
      // High Detail vs. Abstract Crop logic
  ...
  return (
    <div style={{ backgroundColor: '#000', minHeight: '100vh', padding: '20px', color: '#FFF', fontFamily: 'monospace' }}>
      <Head><title>VARIABLE_SCALE_V1.8.2</title></Head>
      ...
      <div style={{ maxWidth: '800px', margin: '0 auto', border: '1px solid #FFF', padding: '20px' }}>
        {!generatedImg ? (
          <>
            <h1 style={{ fontSize: '1.2rem', borderBottom: '1px solid #FFF', paddingBottom: '10px' }}>BLENDER_SCALE_SYNTH</h1>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '10px', marginTop: '20px' }}>
              {selections.map((s, i) => (
                <button key={i} onClick={() => setActiveSlot(i)} style={{ height: '80px', background: s?.color || '#111', border: '1px solid #FFF', color: '#FFF', cursor: 'pointer', fontWeight: 'bold' }}>
                  {s ? s.code : `SLOT_0${i+1}`}
                </button>
              ))}
            </div>
          ...
          <div style={{ textAlign: 'center' }}>
            ...
            <button onClick={() => setGeneratedImg(null)} style={{ marginTop: '20px', padding: '10px 20px', background: '#FFF', color: '#000', fontWeight: 'bold', cursor: 'pointer' }}>NEW_REITERATION</button>
          </div>
        )}
      </div>
